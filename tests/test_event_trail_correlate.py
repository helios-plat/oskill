...
if True:
    from oskill.event_trail_correlate import event_trail_correlate
...
def test_event_trail_correlate_no_timestamp():
    ...
    events = [{"id": "E1"}, {"id": "E2", "parent_id": "E1"}]
    result = event_trail_correlate(target_event_id="E1", all_events=events)
    assert len(result.causally_related) == 1
...
def test_event_trail_correlate_custom_keys():
    ...
    result = event_trail_correlate(
        target_event_id="E1", all_events=events, causal_keys=("caused_by",)
    )
    assert len(result.causally_related) == 1
    assert result.causally_related[0]["id"] == "E2"
...
def test_backward_compat_no_change_param():
    ...
    r = event_trail_correlate(target_event_id="E1", all_events=_CHANGE_EVENTS, time_window_sec=300)
