    ...
    mock_llm.side_effect = [
        {
            ...
            "tool_calls": [
                {"id": "1", "name": "oprim__docker_container_logs", "input": {"container_id": "c1"}}
            ],
        },
        {
...
def test_agentic_investigate_loop_max_steps():
    ...
    result = agentic_investigate_loop(
        signal=signal, available_tool_names=["oprim.dummy_tool"], llm=mock_llm, max_steps=3
    )
...
def test_agentic_investigate_loop_llm_error():
    ...
    result = agentic_investigate_loop(signal=signal, available_tool_names=[], llm=mock_llm)
...
def test_agentic_investigate_loop_tool_not_found():
    ...
    signal = Signal(source="test", severity="warning")
    result = agentic_investigate_loop(
        signal=signal, available_tool_names=["missing_tool"], llm=mock_llm, max_steps=1
    )
...
def test_agentic_investigate_loop_tool_exception():
    ...
    signal = Signal(source="test", severity="warning")
    result = agentic_investigate_loop(
        signal=signal, available_tool_names=["oprim.fail_tool"], llm=mock_llm, max_steps=1
    )
...
def test_agentic_investigate_loop_on_step_callback():
    ...
    signal = Signal(source="test", severity="warning")
    agentic_investigate_loop(
        signal=signal, available_tool_names=["oprim.t1"], llm=mock_llm, on_step=on_step
    )
...
def test_agentic_investigate_loop_no_available_tools():
    ...
    signal = Signal(source="test", severity="warning")
    result = agentic_investigate_loop(
        signal=signal, available_tool_names=["non_existent_tool"], llm=mock_llm
    )
    assert result.stopped_reason == "llm_decided_stop"
