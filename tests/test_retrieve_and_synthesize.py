...
if True:
    from oskill.retrieve_and_synthesize import RetrievedDoc, retrieve_and_synthesize
...
def test_retrieve_and_synthesize_success():
    ...
    result = retrieve_and_synthesize(
        query="database error", corpus_id="incidents", llm=mock_llm, vector_search_fn=mock_search
    )
...
def test_retrieve_and_synthesize_no_docs():
    ...
    result = retrieve_and_synthesize(
        query="missing topic", corpus_id="empty_corpus", llm=mock_llm, vector_search_fn=mock_search
    )
...
def test_retrieve_and_synthesize_llm_failure():
    ...
    with pytest.raises(Exception, match="LLM Error"):
        retrieve_and_synthesize(
            query="test", corpus_id="c1", llm=mock_llm, vector_search_fn=mock_search
        )
...
def test_retrieve_and_synthesize_low_confidence():
    ...
    result = retrieve_and_synthesize(
        query="vague query", corpus_id="c1", llm=mock_llm, vector_search_fn=mock_search
    )
...
def test_retrieve_and_synthesize_top_k():
    ...
    retrieve_and_synthesize(
        query="q", corpus_id="c", llm=mock_llm, vector_search_fn=mock_search, top_k=3
    )
...
def test_retrieve_and_synthesize_empty_query():
    ...
    result = retrieve_and_synthesize(
        query="", corpus_id="c", llm=mock_llm, vector_search_fn=mock_search
    )
    assert result.synthesized_answer == "No relevant documents found."
...
def test_retrieve_and_synthesize_complex_content():
    ...
    mock_llm.return_value = {
        "content": 'Answer with quotes "quoted" and newlines\n\nConfidence: 1.0'
    }
    ...
    result = retrieve_and_synthesize(
        query="q", corpus_id="c", llm=mock_llm, vector_search_fn=mock_search
    )
