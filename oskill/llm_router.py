...
class LLMRouter:
    ...
    def route(
        ...
        *,
        ...
        decision = route_decision(messages, tools, matrix, priority=priority, budget=budget)
    ...
    async def dispatch_long(
        ...
        *,
        ...
        if planner is not None:
            ...
            self._audit_write(
                {"action": "long_plan", "ts": time.time(), "plan_items": len(plan or [])}
            )
        ...
        # 无规划器 → 原规则切分并行
        result = await dispatch_parallel(prompt, caller, max_parallel=parallelism, title=title)
        result["alias"] = matrix.get("alias", "veya1.2")
        self._audit_write(
            {
                "action": "dispatch_long",
                "ts": time.time(),
                **{k: v for k, v in result.items() if k in ("parallel", "chunks", "elapsed_s")},
            }
        )
        return result
    ...
    async def call_aliased(
        ...
        *,
        ...
        if decision["route"] == "long":
            ...
            return await self.dispatch_long(
                prompt, _chunk_caller, title="长文并行回答", planner=planner_caller
            )
        result = await caller(
            {
                "provider": decision["provider"],
