...
from oskill.knowledge._pg_meta import (
    detect_bundle_duplicate_pg,
    pg_enabled,
    write_substrate_pg,
)
...
async def _detect_bundle_duplicate(bundle_file_hash: str, user_id_hash: str) -> int:
    ...
    try:
        if pg_enabled():
            return detect_bundle_duplicate_pg(bundle_file_hash, user_id_hash)
        db_p = meta_db_path()
        if not db_p.exists():
            return 0
...
async def ingest_substrate(
    ...
    # Step 9: write meta store — PostgreSQL when configured (STRATUM_PG_*),
    # else legacy DuckDB meta_db (being retired).
    ...
    if pg_enabled():
        write_substrate_pg(
            ...
            user_id_hash=user_id_hash,
            title=title,
            mime=path_mime or None,
            source_path=str(dest),
            file_hash=file_hash if content_override is None else None,
            byte_size=path.stat().st_size,
            meta_json=_meta_dict,
